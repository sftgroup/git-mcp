...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if len(parts) >= 2 and parts[0] == "raw-upload":
            ...
            if keep_git:
                subprocess.run("find "+local+" -maxdepth 1 ! -name .git ! -path "+local+" -exec rm -rf {} +", shell=True, timeout=5)
            else:
                subprocess.run("rm -rf "+local+"/*", shell=True, timeout=5)
            ...
            self.end_headers()
            result = '{"ok":true,"team":"'+repo+'","uploadSizeMB":"'+format(content_len/1048576,".2f")+'","hint":"Use git_push to commit, then git_sync to push to GitHub"}'
            self.wfile.write(result.encode())
        else:
            self.send_error(404)

print("Raw server on :"+str(PORT))
