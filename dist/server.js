...
const tools = {
    ...
    code_upload: { handler: apiCodeUpload, description: "Get upload URL for raw HTTP upload (no token cost). Upload tar.gz via curl, then call git_push to commit.", inputSchema: { type: "object", properties: { team: { type: "string", description: "Repo name" }, branch: { type: "string" } }, required: ["team"] } },
...
async function mcpHandler(req, res) {
    ...
    const send = (r) => res.json({ jsonrpc: "2.0", result: r, id });
    const fail = (c, m) => res.json({ jsonrpc: "2.0", error: { code: c, message: m }, id });
