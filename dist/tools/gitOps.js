...
import { registerRepo, listRepos, getRepo, createTag, listTags, getLatestTag, logAudit, listAudit } from "../db.js";
const cfg = loadConfig();
// ─── Git helpers ──────────────────────────────────────
function gitTokenUrl(url) {
    const token = process.env.GIT_TOKEN ?? process.env.GITHUB_TOKEN ?? "";
    if (!token)
        return url;
    return url.replace("https://", `https://${token}@`);
}
function git(cwd, cmd, timeoutSec = 60) {
    return execSync(`git ${cmd}`, { cwd, timeout: timeoutSec * 1000, maxBuffer: 5 * 1024 * 1024 }).toString().trim();
}
function gitOptional(cwd, cmd, timeoutSec = 30) {
    try {
        return git(cwd, cmd, timeoutSec);
    }
    catch {
        return "";
    }
...
export async function apiRegisterRepo(input) {
    if (getRepo(input.name))
        throw new Error(`Repo "${input.name}" already registered`);
...
export async function apiListRepos(input) {
    const repos = listRepos(input.search);
    return { repos, total: repos.length };
}
export async function apiGetRepo(input) {
    const repo = getRepo(input.name);
    if (!repo)
        throw new Error(`Repo "${input.name}" not found. Use repo_list to see available repos.`);
...
export async function apiCreateGithubRepo(input) {
    const org = cfg.githubOrg;
    const token = process.env.GIT_TOKEN ?? process.env.GITHUB_TOKEN;
    if (!token)
        throw new Error("GIT_TOKEN env var not set");
    ...
    const result = execSync(`curl -s -X POST "${url}" -H "Authorization: token ${token}" -H "Content-Type: application/json" -d '${body}'`, { timeout: 30000 }).toString();
...
export async function apiClone(input) {
    const repo = getRepo(input.name);
    if (!repo)
        throw new Error(`Repo "${input.name}" not found`);
...
export async function apiPull(input) {
    const repo = getRepo(input.name);
    if (!repo)
        throw new Error(`Repo "${input.name}" not found`);
    if (!existsSync(join(repo.local_path, ".git")))
        throw new Error(`Not cloned. Run git_clone first.`);
    const branch = input.branch ?? repo.default_branch;
...
export async function apiPush(input) {
    ...
    db.getDb().prepare("INSERT INTO sync_log (repo_id, action, commit_sha, message, status) VALUES (?, 'push', ?, ?, 'pending')").run(db.getRepo(input.name)?.id ?? 0, commitSha, input.message);
    return {
...
export async function apiSync(input) {
    const repo = getRepo(input.name);
    if (!repo)
        throw new Error(`Repo "${input.name}" not found`);
    if (!existsSync(join(repo.local_path, ".git")))
    ...
    const token = process.env.GIT_TOKEN ?? process.env.GITHUB_TOKEN;
    if (!token)
        throw new Error("GIT_TOKEN not set — cannot sync to GitHub");
    const branch = input.branch ?? repo.default_branch;
    git(repo.local_path, `checkout ${branch}`);
    ...
    // Update sync_log
    const db = await import("../db.js");
    const repoId = db.getRepo(input.name)?.id ?? 0;
    for (const c of syncCommits) {
        db.getDb().prepare("UPDATE sync_log SET status = 'synced', synced_at = datetime('now') WHERE commit_sha = ? AND status = 'pending'").run(c.sha);
    }
...
export async function apiSyncStatus(input) {
    if (input.name) {
        const repo = getRepo(input.name);
        if (!repo)
            throw new Error(`Repo "${input.name}" not found`);
        if (!existsSync(join(repo.local_path, ".git")))
            return { name: input.name, unsynced: 0, commits: [] };
...
// ─── Status ────────────────────────────────────────────
export async function apiStatus(input) {
    const repo = getRepo(input.name);
    if (!repo)
        throw new Error(`Repo "${input.name}" not found`);
    if (!existsSync(join(repo.local_path, ".git")))
...
export async function apiListTags(input) {
    const tags = listTags(input.name);
    return { tags, total: tags.length };
}
// ─── Log ───────────────────────────────────────────────
export async function apiLog(input) {
    const repo = getRepo(input.name);
    if (!repo)
        throw new Error(`Repo "${input.name}" not found`);
    if (!existsSync(join(repo.local_path, ".git")))
...
export async function apiCheckout(input) {
    ...
    if (status)
        return { ok: false, dirty: true, message: "Uncommitted changes. Commit or stash first." };
...
// ─── Integrity Check ──────────────────────────────────
export async function apiCheck(input) {
    const repo = getRepo(input.name);
    if (!repo)
        throw new Error(`Repo "${input.name}" not found`);
    const localPath = repo.local_path;
    if (!existsSync(join(localPath, ".git")))
        throw new Error(`Not cloned.`);
    const guardConfig = JSON.parse(repo.guard_config ?? "{}");
    const results = {};
    let allPassed = true;
...
export async function apiSyncCode(input) {
    ...
    // Count files synced
    const fileCount = parseInt(execSync(`find ${destPath} -type f | wc -l`, { timeout: 5000 }).toString().trim()) || 0;
    const totalBytes = parseInt(execSync(`du -sb ${destPath} | cut -f1`, { timeout: 5000 }).toString().trim()) || 0;
    ...
    if (existsSync(join(destPath, ".git"))) {
        ...
        sha = execSync(`find ${destPath} -type f -exec sha256sum {} \\; | sort -k2 | sha256sum | cut -d' ' -f1`, { timeout: 30000 }).toString().trim();
    }
...
export async function apiSnapshot(input) {
...
export async function apiRepoPull(input) {
    ...
    try {
        ...
        const repo = getRepo(team);
        if (!repo)
            throw new Error("Repo not found: " + team);
        const ghUrl = repo.github_url;
...
export async function apiCodeUpload(input) {
...
export async function apiCodeExport(input) {
    const localPath = join(cfg.repoBasePath, input.team);
    if (!existsSync(localPath)) {
